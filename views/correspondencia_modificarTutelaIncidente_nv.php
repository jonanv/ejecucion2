...
$(document).ready(function() {
	...
	$(".generarword").click(function(){
		...
		var idactuacion	   = $(this).attr('data-idactuacion');
		
		var idpa_actuacion = $(this).attr('data-idpa_actuacion');
			
		//alert(idactuacion);
		
		location.href="index.php?controller=correspondencia&action=Generar_Documento_Correspondencia&opcion=3&idactuacion="+idactuacion+"&datos0c_independiente="+1000;
  ...
  frm.cantidad_detalles.value=numf;
   ...
   //---------------PROYECTO OFICIOS TUTELAS, ADICIONADO POR JORGE ANDRES VALENCIA, 8 DE FEBRERO 2018------------------
	...
	ele          = document.createElement('textarea'); 
   	//ele.type     = '<label>'; 
   	ele.name     = 'descripA'+numf; 
   	//ele.value    = 'DESCRIPCION: ';
   	ele.id       = 'descripA'+numf;
	ele.cols     = '100';
	ele.rows     = '20';
	ele.title    = 'descripA'+numf;
   	//ele.disabled = 'true';
   	contenedor.appendChild(ele);
...
if(m=='Tutela')
{
 ...
 actuaciones = new Array("Admisión","Archivo","Avoca Conocimiento","Confirmación fallo","Confirmación parcial","Corrección","Desistimiento","Envio a la Corte Costitucional","Envio por competencia","Fallo 1° Instancia","Fallo 2° Instancia","Impugnación","Inadmisión","Medida previa","Niega pruebas","Pruebas","Rechazo","vinculación");
  idactuaciones = new Array("33","1","3","6","7","8","35","9","10","11","12","15","16","17","18","21","22","28");
 }
 if(m=='Incidente')
{
...
function validar_campo_accionante_acc_vin(id,num) {
    ...
    valor_otro = document.getElementById('accionante_accionado_vinculado_otro-'+numero).checked; 
	  ...
	  //Validación cuando el usuario selecciona "Accionado" 
	  if (valor_accionado)
  	  {
       //alert("accionado");
      ...
      else
	  {
	    ...
	    contador=tmp.length;
	    a = 0;
	    //Crear lista de selección de accionados
		...
		//Adicionar a la lista de selección de accionados las opciones que vienen de base de datos
	   if(primera_opt.length >0)
	   {
	    ...
	    //Obtener el campo de accionado del formulario, y adicionarlo a la lista de selección
		...
		//Adicionar a la lista de selección los accionados adicionados por el formulario	  
	    while (j<=cant_opt)
	    {  
	     opcion = opciones[k];
...
/****  Remover del formulario los campos de: accionante, la lista de accionados si estan definidos   ****/
  ...
  /****  Crear la lista de selección de vinculados:    ****/
	  ...
	  //var primera_opt = document.frm.vinculado.value;
	   //Crear lista de selección de vinculados
	  ele = elemento=document.createElement('select'); 
      tipo_act=ele;
	  ...
	  //Adicionar a la lista de selección de vinculados las opciones que vienen de base de datos
	 if(primera_opt_vinculado.length >0)
	 {
	  ...
	  //Obtener el campo de vinculado del formulario, y adicionarlo a la lista de selección
	  ...
	  //Adicionar a la lista de selección los vinculados adicionados por el formulario	  
...
function Recargar_Vista(dat_1) {

	location.href="index.php?controller=correspondencia&action=edit_correspondenciaTutela&nombre="+dat_1;
...
?>
 ...
 while($field = $datos_correspondencia->fetch()){?>
          ...
          <input type="hidden" name="radicadojxxi" id="radicadojxxi" value="<?php echo $field[radicado]; ?>" />
 ...
 while($field_partes = $datos_partes->fetch()){  
   ...
   ?>
		  ...
		  <tr>
            ...
            <td bgcolor="#EBE9ED">
				<strong>Id Actuacion:</strong>
			</td>
            <td colspan="4" bgcolor="#EBE9ED">
				<strong><?php echo $field_partes[idactuacion];?></strong>
				<a class="generarword" href="javascript:void(0);" data-idactuacion="<?php echo $field_partes[idactuacion];?>"><img src="views/images/w4.png" width="35" height="35" title="GENERAR OFICIO" style="float:right "/></a>
			</td>
          ...
          <tr>
            ...
            <td bgcolor="#EBE9ED">
				...
				$RUTA_GUIA = "http://svc1.sipost.co/trazawebsip2/frmReportTrace.aspx?ShippingCode=".$field_partes[nunguia];
				
				?>
				
				<a class="ruta_guia" data-ruta="<?php echo $RUTA_GUIA; ?>"><?php echo $field_partes[nunguia];?></a>
...
<?php 
		...
		$id_radicado_consultado_AC = explode("******",$id_radicado_consultado);

		
		if($id_radicado_consultado_AC[1] == 1){
		
		
			echo '<script languaje="JavaScript"> 
			
					var dat_1 = "'.$id_radicado_consultado_AC[0].'";
								
					Recargar_Vista(dat_1);
