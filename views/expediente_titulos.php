...
<?php 
	...
	if($opcion == 1){
		...
		$datoJXX6 = trim($_GET['datox6']);//tipo (programacion o consulta de titulos)
		
		$datoJXX8 = trim($_GET['datox8']);//estado titulo (respuesta procesada o no procesada)

		$datosACCION_1 = $modelo->listar_expediente_titulos_filtro($datoJXX1,$datoJXX3,$datoJXX5,$datoJXX2,$datoJXX4,$datoJXX6,$datoJXX8);
			
		//*********************CANTIDAD REGISTROS*****************************************
		
		$datosACCION = $modelo->listar_expediente_titulos_filtro($datoJXX1,$datoJXX3,$datoJXX5,$datoJXX2,$datoJXX4,$datoJXX6,$datoJXX8);
		...
		$datosACCION_1 = $modelo->listar_expediente_titulos();
			
		//*********************CANTIDAD REGISTROS*****************************************
		
		$datosACCION = $modelo->listar_expediente_titulos();
...
$(document).ready(function() {
	...
	$("#radicar_respuesta").click(function(evento){
		...
		if( $('#observacionesm2').val() == null || $('#observacionesm2').val().length == 0 || /^\s+$/.test($('#observacionesm2').val()) ) {
			...
			dataString += '&observacionesm2='+$("#observacionesm2").val();
		...
		//ID TIPO
		if( $('#tipo').val() == null || $('#tipo').val().length == 0 || /^\s+$/.test($('#tipo').val()) ) {
				
			msg = "Defina Tipo(CAMPO OCULTO)";
			$('.mensage_memo').html(msg);//A el div con la clase msg, le insertamos el mensaje en formato  thml
			$('.mensage_memo').show('slow');
		...
		if(validar == 0){
				...
				if(nom_archivo == 0){
					...
					data.append('observacionesm2',$('#observacionesm2').val());
					...
					if (confirm ("ESTA SEGURO DE REALIZAR EL PROCESO")) {
											...
											$.ajax({
												...
												url:'index.php?controller=archivo&action=Radicar_Respuesta',
												...
												setTimeout(function() {
													...
													//RECARGAR 
													location.href='index.php?controller=archivo&action=Programacion_Consulta_Titulos';
	...
	$('#exampleModal_5').on('show.bs.modal', function (event) {
		...
		if(recipient_2[14] >= 1){
			...
			msg_respuesta = recipient_2[15]+' '+recipient_2[16];
		}
		else{
		...
		if(recipient_2[5] == 1){
			...
			if(recipient_2[12] == 'P'){
	  			...
	  			modal.find('.modal-title').text('ID: '+ recipient_2[8] +' - PROGRAMAR PAGO DE TITULOS: ' + recipient_2[0] +' / EXPEDIENTE ESTA DIGITALIZADO / RESPUESTA PROCESADA: '+msg_respuesta)
				
			}
			
			if(recipient_2[12] == 'C'){
		
	  			modal.find('.modal-title').text('ID: '+ recipient_2[8] +' - CONSULTA DE TITULOS: ' + recipient_2[0] +' / EXPEDIENTE ESTA DIGITALIZADO / RESPUESTA PROCESADA: '+msg_respuesta)
			...
			if(recipient_2[12] == 'P'){
	  			...
	  			modal.find('.modal-title').text('ID: '+ recipient_2[8] +' - PROGRAMAR PAGO DE TITULOS: ' + recipient_2[0] +' / EXPEDIENTE NO ESTA DIGITALIZADO / RESPUESTA PROCESADA: '+msg_respuesta)
				
			}
			
			if(recipient_2[12] == 'C'){
		
	  			modal.find('.modal-title').text('ID: '+ recipient_2[8] +' - CONSULTA DE TITULOS: ' + recipient_2[0] +' / EXPEDIENTE NO ESTA DIGITALIZADO / RESPUESTA PROCESADA: '+msg_respuesta)
		...
		$('#nombre').val(recipient_2[1]);         //ID RADICAD
		$('#idmemoexterno').val(recipient_2[8]);//ID MEMORIAL EXTERNO
		$('#tipo').val(recipient_2[12]);//TIPO PROGRAMAR O CONSULTAR TITULO

		//CARGA INFORMACION SI YA FUE PROCESADA LA RESPUESTA
		//Y DESEAN EDITARLA
		$('#observacionesm2').val('');
		$('#observacionesm2').val(recipient_2[13]);
	...
	$('#exampleModal_asunto').on('show.bs.modal', function (event) {
		...
		modal.find('.modal-title').text('ID: '+ recipient_2[0] +' - EXPEDIENTE: ' + recipient_2[1])
				
		
	  	//modal.find('.modal-body input').val(recipient)
		
		$('#asunto2').val('');
		$('#asunto2').val(recipient_2[2]);
	...
	$("#buscar_pro").click(function(evento){
			...
			if( 
				...
				location.href="index.php?controller=archivo&action=Busquedad_Titulos_Consulta&dato_0="+dato_0+"&dato_1="+dato_1+"&dato_2="+dato_2+"&datox1="+datox1+"&datox2="+datox2+"&datox4="+datox4+"&datox6="+datox6+"&datox8="+datox8;
...
<center><h2 class="page-header">PROGRAMACION PAGO Y CONSULTA DE TITULOS</h2></center>
		...
		<td>
			...
			<div class="form-row">
				...
				<div class="form-group col-md-6">
					  ...
					  <label for="input_3">Estado Titulo</label>
					 
					  <select class="form-control" name="filtro8" id="filtro8">
																
						<option value="" selected="selected">Seleccionar Estado Titulo</option> 
						
						<option value="0">Respuesta Procesada</option> 
						<option value="1">Respuesta Sin Procesar</option> 
...
<!-- class="table-responsive" PARA BARRA DE DESPLAZAMIENTO EN LA TABLA Y SE AJUSTE A CUALQUIER DISPOSITIVO -->
...
<table class="table table-striped table-bordered table table-hover" id="tsoli">
    <thead>
        <tr class="success">
			...
			<th style="width:120px; text-align:center">ASUNTO</th>
	...
	<?php
			...
			while($fila = $datosACCION_1->fetch()){
				...
				//SE REALIZA ESTA PREGUNTA PARA QUE LOS REGISTROS QUE NO TENGAN
				//UN ARCHIVO DEFINIDO NO MUESTRE EL ICONO PDF
				if(empty($d6M)){
				...
				$d17M  = $fila[respuestaoecm];
				$d18M  = $fila[iduserrespuesta];
				$d19M  = $fila[fecharespuesta];
				$d20M  = $fila[horarespuesta];
				
				$d21M  = $fila[celular];
				
				$d22M  = $fila[ruta_respuesta];
        ...
        <tr>
				...
				<td style="width:120px; text-align:center"> 
					...
					<?php 
						...
						echo utf8_encode($d8M)."<br>"."<br>".utf8_encode($d21M); //correo / celular
				...
				<td style="width:120px; text-align:center"> 
					...
					<?php //echo utf8_encode($d5M); //asunto?>
					
					<button type="button" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#exampleModal_asunto" data-whatever="<?php echo utf8_encode($d0M."*/*/*".$d1M."*/*/*".$d5M); ?>">
				...
				<td style="width:120px; text-align:center"> 
					...
					<?php 
						...
						if($d16M == "P"){
							...
							$msgaccion = "PROGRAMACION"."<br>"."PAGO DE TITULOS";
					
					?>
							<button type="button" class="btn btn-danger btn-xs"><?php echo $msgaccion; ?></button>
					...
					<?php 
						...
						if($d16M == "C"){
							...
							$msgaccion = "CONSULTA"."<br>"."DE TITULOS";
							
					?>
					
							<button type="button" class="btn btn-info btn-xs"><?php echo $msgaccion; ?></button>
				...
				<td style="width:120px; text-align:center"> 
					...
					<button type="button" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#exampleModal_5" data-whatever="<?php echo $d1M."*/*/*".$d14M."*/*/*".$d2M."*/*/*".$d13M."*/*/*".$d6M."*/*/*".$d12M."*/*/*".$d8M."*/*/*".$d10M."*/*/*".$d0M."*/*/*".$d7M."*/*/*".$d9M."*/*/*".$d11M."*/*/*".$d16M."*/*/*".$d17M."*/*/*".$d18M."*/*/*".$d19M."*/*/*".$d20M; ?>">
				...
				<td style="width:120px; text-align:center"> 
					...
					<?php 
						//$msg_memo = "RESPUESTA PROCESADA";
						
					?>
						
						
						<button type="button" class="btn btn-success btn-xs" data-toggle="modal" data-target="#exampleModal_5" data-whatever="<?php echo utf8_encode($d1M."*/*/*".$d14M."*/*/*".$d2M."*/*/*".$d13M."*/*/*".$d6M."*/*/*".$d12M."*/*/*".$d8M."*/*/*".$d10M."*/*/*".$d0M."*/*/*".$d7M."*/*/*".$d9M."*/*/*".$d11M."*/*/*".$d16M."*/*/*".$d17M."*/*/*".$d18M."*/*/*".$d19M."*/*/*".$d20M); ?>">
...
<div class="modal fade" id="exampleModal_5" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <div class="modal-dialog" role="document" id="mdialTamanio_5">
    ...
    <div class="modal-content">
     ...
     <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">TITULOS</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>  
			...
			<table class="table"> 
					...
					<td>
						...
						<div class="form-row">
							...
							<div class="form-group col-md-4">
								  ...
								  <br>
								  <textarea name="observacionesm2" id="observacionesm2" cols="90" rows="10" maxlength = "1000000" ></textarea>
				...
				<tr>
					...
					<td>
						...
						<a id="radicar_respuesta" title="DAR RESPUESTA">
				...
				<tr>
					...
					<td>
								 ...
								 <!-- CAMPOS OCULTOS -->
								  ...
								  <br>
								  <input type="hidden" class="form-control" name="tipo" id="tipo" readonly="true">
...
<div class="modal fade" id="exampleModal_asunto" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
					...
					<td>
						...
						<div class="form-row">
							...
							<div class="form-group col-md-4">
								  ...
								  <br>
								  <textarea name="asunto2" id="asunto2" cols="90" rows="10" maxlength = "1000000" ></textarea>
