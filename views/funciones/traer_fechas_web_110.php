...
while($if < 3){
	...
	//SE REALIZA ESTA PREGUNTA PARA QUE ASIGNE
	//EN FECHA DE FIJACION $fechat YA QUE 
	//CON LOS DATOS CALCULADOS EN if($if == 0)
	//SE REALIZA ESTE CALCULO UNA FECHA MAS 
	//A LA FECHA DEL REGISTRO DE LA LIQUIDACION
	//Y CON ESTA LAS DOS FECHAS SIGUINETES INICIAL Y FNAL
	//DEL TRASLADO 
	if($if == 0){
	
		$fecha_f  = explode(" ",$cadenafechas);
		$fecha_2f = explode("/",$fecha_f[0]);
	
		$fechat = trim($fecha_2f[2]."-".$fecha_2f[1]."-".$fecha_2f[0]);
		
		
		//$fechat = trim("2017-05-24");
	}
