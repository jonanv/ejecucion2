...
<?php 
	...
	//VARIABLES QUE CONTROLAN SI UN EXPEDIENTE ESTA BLOQUEADO O NO
	$expediente_bloqueado_1 = $modelo->Expediente_Bloqueado($id_radi);
	$expediente_bloqueado_2 = $expediente_bloqueado_1->fetch();
	$expediente_bloqueado_3	= trim($expediente_bloqueado_2[expebloqueado]);
	
	$msgbloqueo = "";
	if($expediente_bloqueado_3 >= 1){
		$msgbloqueo = "BLOQUEADO";
	}
	...
	/*$nombrelista_6    = 'expe_cuaderno';
	$campoordenar_6   = 'id';
	$datosCUADERNOS_6 = $modelo->get_lista($nombrelista_2,$campoordenar_2);*/
...
$idaccion	  = '50';
$campoordenar = 'id';
$datos_50_1   = $modelo->get_lista_usuario_acciones($campos,$nombrelista,$idaccion,$campoordenar);
$datos_50_2   = $datos_50_1->fetch();
$datos_50_3	  = explode("////",$datos_50_2[usuario]);
		
$bandera_50   = 0;
		
if ( in_array($_SESSION['idUsuario'],$datos_50_3,true) ){
...
//******************ID USUARIOS, QUE VISUALIZAN BOTON PARA RENOMBRAR LOS ARCHIVOS DE UN EXPEDIENTE***************
$idaccion	  = '51';
$campoordenar = 'id';
$datos_51_1   = $modelo->get_lista_usuario_acciones($campos,$nombrelista,$idaccion,$campoordenar);
$datos_51_2   = $datos_51_1->fetch();
$datos_51_3	  = explode("////",$datos_51_2[usuario]);
		
$bandera_51   = 0;
		
if ( in_array($_SESSION['idUsuario'],$datos_51_3,true) ){
		
	$bandera_51 = 1;
}	
//******************FIN ID USUARIOS, QUE VISUALIZAN BOTON PARA RENOMBRAR LOS ARCHIVOS DE UN EXPEDIENTE***************


//******************ID USUARIOS, QUE VISUALIZAN EN EL EXPEDIENTE DIGITAL SI ALGUN ARCHIVO NO ES ENCONTRADO EN LA UBICACION QUE TIENE GUARDADA EN SU COLUMNA ruta DE LA TABLA expe_digital***************
$idaccion	  = '53';
$campoordenar = 'id';
$datos_53_1   = $modelo->get_lista_usuario_acciones($campos,$nombrelista,$idaccion,$campoordenar);
$datos_53_2   = $datos_53_1->fetch();
$datos_53_3	  = explode("////",$datos_53_2[usuario]);
		
$bandera_53   = 0;
		
if ( in_array($_SESSION['idUsuario'],$datos_53_3,true) ){
		
	$bandera_53 = 1;
}	
//******************FIN ID USUARIOS, QUE VISUALIZAN EN EL EXPEDIENTE DIGITAL SI ALGUN ARCHIVO NO ES ENCONTRADO EN LA UBICACION QUE TIENE GUARDADA EN SU COLUMNA ruta DE LA TABLA expe_digital***************
...
$(document).ready(function() {
	...
	$('#exampleModal_200').on('show.bs.modal', function (event) {
		...
		// Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
		var modal = $(this)
		modal.find('.modal-title').text('EXPORTAR A EXCEL INDICE ELECTRONICO EXPEDIENTE: ' + recipient_2[0])
		//modal.find('.modal-body input').val(recipient)
		
		$('#cuadernoEXP').html('');
		$("#cuadernoEXP").load('funciones/traer_datos_lista.php?id='+recipient_2[1]+"&idsql="+8);
		
		$('#id_radi_excel').val(recipient_2[1]);
				
				
	})//FIN $('#exampleModal').on('show.bs.modal', function (event) {
	
	$('#exampleModal_300').on('show.bs.modal', function (event) {
	 	 ...
	 	 // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
	  	var modal = $(this)
	  	modal.find('.modal-title').text('DETALLE EXPEDIENTE: ' + recipient)
	  	//modal.find('.modal-body input').val(recipient)
		...
		$.ajax({
			type: "GET",
			//url: "views/popupbox/expediente_digital_partes_JXXI.php?tabla=1",
			url: "funciones/expediente_digital_datos_JXXI.php?tabla=1",
			data: { recipient: recipient }
		})
		.done(function(json) {
			...
			for(var i=0;i<json.length;i++)
			{
				...
				registro+="<tr>"
					...
					registro+="<td class='A112NUMESUJE' style='text-align:center'>"+json[i].A103CODIPROC+"</td>"
					registro+="<td class='A112NOMBSUJE' style='text-align:center'>"+json[i].A052DESCPROC+"</td>"
					registro+="<td class='A112NUMESUJE' style='text-align:center'>"+json[i].A103ENTIRADI+"</td>"
					registro+="<td class='A112NOMBSUJE' style='text-align:center'>"+json[i].A051DESCENTI+"</td>"
					registro+="<td class='A112NUMESUJE' style='text-align:center'>"+json[i].A103ESPERADI+"</td>"
					registro+="<td class='A112NOMBSUJE' style='text-align:center'>"+json[i].A062DESCESPE+"</td>"
					registro+="<td class='A112NUMESUJE' style='text-align:center'>"+json[i].A103CODICLAS+"</td>"
					registro+="<td class='A112NOMBSUJE' style='text-align:center'>"+json[i].A053DESCCLAS+"</td>"
					registro+="<td class='A112NUMESUJE' style='text-align:center'>"+json[i].A103CODISUBC+"</td>"
					registro+="<td class='A112NOMBSUJE' style='text-align:center'>"+json[i].A071DESCSUBC+"</td>"
					registro+="<td class='A112NUMESUJE' style='text-align:center'>"+json[i].A103CODIPONE+"</td>"
					registro+="<td class='A112NOMBSUJE' style='text-align:center'>"+json[i].A103NOMBPONE+"</td>"
					
				
				registro+="</tr>"
					
					
				$('.editinplace_300').append(registro);
	...
	$('#exampleModal_400').on('show.bs.modal', function (event) {
	 	 ...
	 	 // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
	  	var modal = $(this)
	  	modal.find('.modal-title').text('PARTES EXPEDIENTE: ' + recipient)
	  	//modal.find('.modal-body input').val(recipient)
		...
		$.ajax({
			type: "GET",
			//url: "views/popupbox/expediente_digital_partes_JXXI.php?tabla=1",
			url: "funciones/expediente_digital_partes_JXXI.php?tabla=1",
			data: { recipient: recipient }
		})
		.done(function(json) {
			...
			for(var i=0;i<json.length;i++)
			{
				...
				if(json[i].A112CODISUJE == "0001"){
					parte = "DEMANDANTE";
				}
				if(json[i].A112CODISUJE == "0002"){
					parte = "DEMANDADO";
				}
				
				registro+="<tr>"
						
					//registro+="<td class='A112LLAVPROC'>"+json[i].A112LLAVPROC+"</td>"
					registro+="<td class='A112CODISUJE' style='text-align:center'>"+parte+"</td>"
					registro+="<td class='A112NUMESUJE' style='text-align:center'>"+json[i].A112NUMESUJE+"</td>"
					registro+="<td class='A112NOMBSUJE' style='text-align:center'>"+json[i].A112NOMBSUJE+"</td>"
					
				
				registro+="</tr>"
					
					
				$('.editinplace_400').append(registro);
	...
	$("#exportar_indice").click(function(evento){
		...
		//SI SELECCIONO 
		//Todos los Indices Electronicos de los Cuadernos del Expediente
		if( $("#checkIE").is(':checked') ) { 
		
			dato_0  = 4;//para saber que es el reporte 1
			tfiltro = 2;//con filtro
				
			datox1 = $("#id_radi_excel").val();
		
			location.href="index.php?controller=archivo&action=Generar_Excel_Expediente&opcion="+dato_0+"&datox1="+datox1;
		
		}
		else{
		
			if( $('#cuadernoEXP').val() == null || $('#cuadernoEXP').val().length == 0 || /^\s+$/.test($('#cuadernoEXP').val()) ) {
				...
				dataString += '&cuadernoEXP='+$("#cuadernoEXP").val();
						
			}
			
			if( $('#id_radi_excel').val() == null || $('#id_radi_excel').val().length == 0 || /^\s+$/.test($('#id_radi_excel').val()) ) {
				...
				dataString += '&id_radi_excel='+$("#id_radi_excel").val();
			...
			if(validar == 0){
				...
				datox1 = $("#id_radi_excel").val();
				datox2 = $("#cuadernoEXP").val();
				
				datox3 = $('#cuadernoEXP option:selected').text();
			
				//alert(datox1+" - "+datox2);
				
				location.href="index.php?controller=archivo&action=Generar_Excel_Expediente&opcion="+dato_0+"&datox1="+datox1+"&datox2="+datox2+"&datox3="+datox3;
				...
				if( 
				   ...
				   $('#filtro3').val().length      == 0 
					...
					if( $("#checkEstado").is(':checked') ) { 
						...
						location.href="index.php?controller=archivo&action=ReporteExcel&opcion="+opcion+"&dato_1="+dato_1+"&dato_2="+dato_2+"&datox7="+datox7+"&datosexp_2="+datosexp_2;
...
function Eliminar_Tabla(idtabla){
	...
	if(idtabla == 300){
		...
		var r;
		var cantidad_filas;
		var TABLA = document.getElementById('tdetalle');
	...
	if(idtabla == 400){
		...
		var r;
		var cantidad_filas;
		var TABLA = document.getElementById('tpartes');
...
<input type="hidden" name="id_radi_excel" id="id_radi_excel" readonly="true"/>
...
<table class="table table-striped table-bordered table table-hover">
	...
	<tr align="center">
		...
		<td>
			...
			<h2><?php echo "EXPEDIENTE DIGITAL ".$msgbloqueo; ?></h2>
	...
	<tr align="center">
	 	...
	 	<td>
			...
			if($bandera_50 == 1){
			?>
			<a onclick="javascript:return confirm('!ESTA SEGURO DE ORDENAR EL EXPEDIENTE! POR FAVOR VERIFICAR SU ORDEN DE FORMA VISUAL, ANTES DE CORRER ESTE PROCEDIMIENTO PARA EVITAR INCONSISTENCIAS.');" href="index.php?controller=archivo&amp;action=Ordenar_Expediente&datosexpOEX=<?php echo $id_radi."******".$nradi; ?>">
				 
				<button type="button" class="btn btn-primary">
					<span class="glyphicon glyphicon-folder-open"></span> Ordenar Expediente
				</button>
			...
			if($bandera_51 == 1){
			?>
			<a href="index.php?controller=archivo&amp;action=Renombrar_Archivos_Expediente&datosINDICE=<?php echo $id_radi."******".$nradi; ?>">
				 
				<button type="button" class="btn btn-primary">
					<span class="glyphicon glyphicon-font"></span> Renombrar Archivos
				</button>
			...
			<?php
			}//FIN ELSE DEL IF if($expediente_bloqueado_3 >= 1){
			?>
			<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal_200" data-whatever="<?php echo $nradi."*/*/*".$id_radi;?>">
					
				<span class="glyphicon glyphicon-share-alt"></span> Exportar a Excel Indice Electronico 
	...
	<tr align="center">
		...
		<td>
			...
			<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal_300" data-whatever="<?php echo $nradi; ?>">
					
				<span class="glyphicon glyphicon-folder-close"></span> Detalle Expediente
					
			</button>		
			
			<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal_400" data-whatever="<?php echo $nradi; ?>">
									
				<span class="glyphicon glyphicon-user"></span> Partes Expediente
...
<h2 class="page-header">
	...
	<?php //echo "EXPEDIENTE DIGITAL ".$msgbloqueo; ?>
			...
			<div class="col-xs-8">
				...
				<div class="form-row">
					...
					<div id="paraestado" class="form-group col-md-6">
						...
						<br>
						<a id="exportar_excel" href="javascript:void(0);">
...
<table class="table table-striped table-bordered table table-hover" id="tsoli">
    <thead>
        <tr class="success">
			...
			<th style="width:80px; text-align:center">N. Documento</th>
			<th style="width:80px; text-align:center">F. Creacion</th>
			...
			?>
				<th style="width:80px; text-align:center">IDRADI</th>
				<th style="width:80px; text-align:center">RADICADO</th>
			<?php
			}
			?>
			
            <th style="width:80px; text-align:center">F. Incorporacion</th>
            <th style="width:80px; text-align:center">Hora</th>
			<!-- <th style="width:80px; text-align:center">Orden Documento</th> -->
          
		    <th style="width:80px; text-align:center">Paginas</th>
			...
			<th style="width:80px; text-align:center">Descripcion</th>
			
			<!-- <th style="width:80px; text-align:center">Formato</th> -->
			<!-- <th style="width:80px; text-align:center"><?php //echo utf8_encode('Tamaño'); ?></th> -->
			<th style="width:80px; text-align:center">Origen</th>
			
			<!-- <th style="width:80px; text-align:center">Observaciones</th> -->
			
			<th style="width:80px; text-align:center">Dependencia</th>
			
			<th style="width:80px; text-align:center">Registra</th>
			<th style="width:80px; text-align:center">Edita</th>
			...
			<?php
			//SE REALIZA ESTA PREGUNTA PARA DETERMINAR SI LA CONSULTA
			//A DEVOLVER ES LA DE ESTADO, AGREGADNCO EL LINK EXPEDIENTE DIGITAL
			if($idradicadoestado_2 >= 1){
			?>
				<th style="width:80px; text-align:center">EXPEDIENTE</th>
			...
			while($fila = $datosACCION_1->fetch()){
				...
				if($idradicadoestado >= 1){
					...
					if($d27M == "NINGUNA"){
						...
						$d18M = $fila[dependencia]." - ".$d27M;
					}
					
					$d19M = $fila[fecha_creacion];
					$d20M = $fila[fecha_de];
					$d21M = $fila[fecha_a];
					$d22M = $fila[origen];
					$d23M = utf8_encode($fila[obs]);
					$d24M = $fila[norigen];
					
					$d25M = $fila[para_estado];
					$d26M = $fila[fecha_estado];
				...
				//NOMBRE DOCUMENTO
				$d9M_2  = explode("/",$fila[ruta]);	
				$d9M_3  = utf8_encode($d9M_2[3]);
				
				$nombre_documento = $modelo->get_nuevo_nombreVISUAL($d9M_3,$fila[orden_documento]);
				
				//NOMBRE DOCUMENTO SIN IDENTIFICADOR
				//ESTO SOLO PARA FORMA VISUAL AL USUARIO
				/*$d9M_6  = explode("-",$d9M_3);
				$longi  = count($d9M_6); 
				
				//SE REALIZA ESTA PREGUNTA, YA QUE ALGUNOS
				//ARCHIVOS SE CARGARON DE FORMA MASIVA AL RPNCIPIO
				//Y SUS NOMBRES DE ARCHIVO NO TIENE IDENTIFICADOR
				if(count($d9M_6) > 1){
				
				
					//SE RECORRE CADA POSICION DEL NOMBRE DEL ARCHIVO
					//201110033718-201110095922-01-201600405RequiereAllegueConstanciaEnvioYEntregaOficioPagador.pdf
					//CON EL FOR $a Y EN $cadena_pos SE CAPTURA  $cadena_pos = 201110033718, $cadena_pos = 201110095922
					//LUEGO SE CONCATENAN LAS DOS PRIMERAS POSICIONES DE $cadena_pos EN $pos_year
					//SI SON >= 20 ESTA PARTE SE QUITA DEL NOMBRE DEL ARCHIVO
					for($a = 0; $a<count($d9M_6); $a++){
					
						$cadena_pos = trim($d9M_6[$a]);
				
						$pos_year = $cadena_pos[0]."".$cadena_pos[1];
							
						if($pos_year >= 20){
							
							//Eliminamos el elemento de la posición 0 "210409112903"
							//EJEMPLO 210409112903-ModificaLiquidacionCredito11201900790.pdf
							//QUEDA ModificaLiquidacionCredito11201900790.pdf
							unset($d9M_6[$a]);
							
						}
						
						//ELIMINO VARIABLES AL FINALIZAR EL FOR
						//Y DENTRO DE EL SEAN ASIGNADAS CON NUEVA INFORMACION
						unset($cadena_pos);
						unset($pos_year);
					...
					//Posición 0 = xxx
					//Posición 1 = xxx
					$d9M_6 = array_values($d9M_6);
					//UNE EL ARRAY EN UN STRING, SEPARADO POR LO QUE SE INDIQUE
					//DENTRO DE LA FUNCION IMPLODE("",XXX)
					$d9M_7 = implode("", $d9M_6);
					
					$nombre_documento = $d9M_7;
				
				}
				else{
					$nombre_documento = $d9M_3;
				}*/
				
				//FORMATO
				$d9M_4  = explode(".",$d9M_3);
				$d9M_5  = strtoupper(utf8_encode($d9M_4[1]));
    ...
    <?php //foreach($this->model->Listar() as $r): ?>
        <tr>
				...
				<?php echo $estilo_cuaderno; ?>
					...
					<?php echo $nombre_documento/*." numero: ".$longi*/; //Nombre Documento?>
			...
			?>
				...
				<?php echo $estilo_cuaderno; ?>
					...
					if($d25M == 1){
						...
						echo $d26M; //Fecha estado
					
					}
					else{
					
						echo $d2M;//Fecha registro
					}
					?>
				...
				<?php echo $estilo_cuaderno; ?>
					...
					<?php echo $d3M; //hora Incorporación Expediente?>
				
				</td>
				
				<?php //echo $estilo_cuaderno; ?>
							
					<?php //echo $contador_registros; //Orden Documento  ?>
			...
			?>
				...
				<?php echo $estilo_cuaderno; ?>
					...
					<?php echo $d4M_2I; //Página Inicio ?>
				
				</td>
				
				<?php echo $estilo_cuaderno; ?>
			
					<?php echo $d4M_2F; //Página Fin ?>
				...
				<?php //echo $estilo_cuaderno; ?>
					...
					<?php //echo $d9M_5; //Formato ?>
				
				<!-- </td> -->
				
				<?php //echo $estilo_cuaderno; ?>
			
					<?php //echo filesize($d9M); //Tamaño ?>
				
				<!-- </td> -->
				
				<?php echo $estilo_cuaderno; ?>
			
					<?php echo utf8_encode($d24M); //Origen ?>
				
				</td>
				
				<?php //echo $estilo_cuaderno; ?>
			
					<?php //echo $d23M; //Observacion ?>
			...
			if ( !empty($d8M) ) {
								...
								//IMAGEN
								//if($d9M_5 == "jpeg" || $d9M_5 == "jpg" || $d9M_5 == "jpe" || $d9M_5 == "jpg2" || $d9M_5 == "tiff"){
								if($d9M_5 == "JPEG" || $d9M_5 == "JPG" || $d9M_5 == "JPE" || $d9M_5 == "JPG2" || $d9M_5 == "TIFF"){
								
									$icono = 'views/images/tipoimagen.png';
								}
								//AUDIO
								if($d9M_5 == "MP3" || $d9M_5 == "WAV"){
								
									$icono = 'views/images/tipoaudio.jpg';
								}
								//VIDEO
								if($d9M_5 == "MPG"  || $d9M_5 == "MP1" || $d9M_5 == "MP2" || $d9M_5 == "MP3" || $d9M_5 == "M1V" || 
								   $d9M_5 == "M1A"  || $d9M_5 == "M2A" || $d9M_5 == "MPA" || $d9M_5 == "MPV" || $d9M_5 == "MP4" || 
								   $d9M_5 == "MPEG" || $d9M_5 == "M4V"){
								
									$icono = 'views/images/tipovideo.png';
								}
								//EXCEL
								if($d9M_5 == "XLS" || $d9M_5 == "XLSX" || $d8M == "application/vnd.ms-excel"){
								
									$icono = 'views/images/excos1.png';
								}
			...
			?>
								...
								//USUARIOS QUE SOLO VISUALIZAN (NO ARCHIVO)
								if ( $bandera_53 == 1 ) {
								
								//ME INDICA QUE NO EXISTE ARCHIVO EN LA UBICACION O RUTA
								//ESPECIFICADA EN LA VARIABLE $d9M = $fila[ruta];
								//SE REALIZA ESTO POR TEMA DE ADMINISTRACION DE ESTE TIPO DE INCONSISTENCIA
								//QUE PUEDE DARSE AL ORDENAR UN EXPEDIENTE O RENOMBRAR SUS ARCHIVOS
								if (!file_exists($d9M)) {
								...
								?>
									<a class="descarga_recibido_entidad" href="javascript:void(0);" title="Recibido" data-idrecibido="<?php echo $d17M; ?>">
									<button type="button" class="btn btn-success btn-xs">Recibido</button>
									</a>
			...
			?>
				...
				<?php echo $estilo_cuaderno; ?>
					...
					<a class="cargar_expe" data-idradicado="<?php echo $d14M;?>" data-radicado="<?php echo $d15M;?>" title="EXPEDIENTE">				
			...
			?>
				...
				<?php echo $estilo_cuaderno; ?>
				  ...
				  <?php echo $estilo_cuaderno; ?>
                	...
                	<a class="glyphicon glyphicon-remove-circle" onclick="javascript:return confirm('Esta Seguro de Eliminar el Registro');" href="index.php?controller=archivo&amp;action=Eliminar_Folio&datosexpELF=<?php echo $id_radi."******".$nradi."******".$d0M."******".$d13M."******".$d11M."******".$d6M; ?>" title="ELIMINAR"></a>
...
<!-- EXPORTAR A EXCEL INCICE ELECTRONICO -->

<div class="modal fade" id="exampleModal_200" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <div class="modal-dialog" role="document" id="mdialTamanio_5">
    ...
    <div class="modal-content">
     ...
     <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel" align="center">EXPORTAR A EXCEL INDICE ELECTORNICO</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>  
			...
			<table class="table"> 
					...
					<td style="border-color:#FFFFFF ">
						...
						<div class="form-row">
							...
							<div class="form-group col-md-6">
								...
								<br>
								<select class="form-control" name="cuadernoEXP" id="cuadernoEXP">
														
									<option value="" selected="selected">Seleccionar Cuaderno</option> 
																						
									<?php
										/*while($row = $datosCUADERNOS_6->fetch()){
								...
								<br>
								 ...
								 <label for="input_1" style="color:#FF0000">Todos los Indices Electronicos de los Cuadernos del Expediente</label>
								 <input type="checkbox" id="checkIE" class="checkbox"/>
				...
				<tr>
					...
					<td>
						...
						<a id="exportar_indice" title="EXPORTAR A EXCEL INDICE ELECTRONICO">
						
							<button type="button" class="btn btn-success" title="EXPORTAR A EXCEL INDICE ELECTRONICO">
								<span class="glyphicon glyphicon-arrow-up"></span><h4>EXPORTAR A EXCEL INDICE ELECTRONICO</h4>
							</button>
...
<div class="modal fade" id="exampleModal_300" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">
     ...
     <div class="modal-header">
      ...
      <div class="modal-body"> 
	  	 ...
	  	 <table id="tdetalle" border="1" class="table table-striped table-bordered table table-hover editinplace_300">
         <!-- <table cellpadding="0" cellspacing="0" rules="rows" border="1" class="editinplace"> -->
				...
				<tr class="success"> 
					...
					<th style="text-align:center">COD</th>
					<th style="text-align:center">CLASE DE PROCESO</th>
					<th style="text-align:center">COD</th>
					<th style="text-align:center">SUBCLASE DE PROCESO</th>
					<th style="text-align:center">COD</th>
					<th style="text-align:center">PONENTE</th>
...
<!-- PARTES DEL PROCESO DESDE JUSTICIA XXI-->

<div class="modal fade" id="exampleModal_400" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <div class="modal-dialog" role="document" id="mdialTamanio">
    ...
    <div class="modal-content">
     ...
     <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">PARTES EXPEDIENTE</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>  
      ...
      <div class="modal-body"> 
	  	 ...
	  	 <table id="tpartes" border="1" class="table table-striped table-bordered table table-hover editinplace_400">
         <!-- <table cellpadding="0" cellspacing="0" rules="rows" border="1" class="editinplace"> -->
