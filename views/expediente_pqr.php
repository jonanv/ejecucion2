<?php
	...
	if ($opcion == 1) {
		...
		$datosACCION_1 = $modelo->listar_expediente_PQR_filtro($datoJXX3, $datoJXX5, $datoJXX2, $datoJXX4);

		//*********************CANTIDAD REGISTROS*****************************************

		$datosACCION = $modelo->listar_expediente_PQR_filtro($datoJXX3, $datoJXX5, $datoJXX2, $datoJXX4);
		...
		$datosACCION_1 = $modelo->listar_expediente_PQR();
		//*********************CANTIDAD REGISTROS*****************************************
		$datosACCION = $modelo->listar_expediente_PQR($datoJXX1);
...
<body>
			...
			<!-- <tr>												
				<td>
					<div class="form-row">
						...
						<input type="number" class="form-control" name="filtro1" id="filtro1" value="<?php //echo trim($_GET['datox1']);
	...
	<table class="table table-bordered table-striped table_datatable" id="tsoli">
		<thead>
			<tr class="success">
				...
				<th data-priority="3">DAR RESPUESTA</th>
			</tr>
		</thead>
		<tbody>
			<?php
				...
				while ($fila = $datosACCION_1->fetch()) {
					...
					$d9M   = $fila["fechaedita"];
					$d10M  = $fila["horaedita"];
					$d11M  = $fila["vizualizar"];
					$d12M  = $fila["celular"];
					$d22M  = $fila["ruta_respuesta"];
			?>
			<tr>
				<td><?php echo $d0M; //id?></td>
				<td><?php echo $d1M; //documento?></td>
				<td><?php echo utf8_encode($d2M); //registra?></td>
				<td><?php echo utf8_encode($d3M) . "<br>" . "<br>" . utf8_encode($d12M); //correo / celular?></td>
				<td><?php echo $d4M; //fecha?></td>
				<td><?php echo $d5M; //hora?></td>
				<td><?php echo $d9M; //fecha edita?></td>
				<td><?php echo $d10M; //hora edita?></td>
				<td>
					<button type="button" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#exampleModal_asunto_pqr" data-whatever="<?php echo utf8_encode($d0M . "*/*/*" . $d1M . " - " . $d2M . "*/*/*" . $d6M); ?>">
						ASUNTO
					</button>
				</td>
				<td>
					<?php
						if ($d11M == 1) {
					?>
						<button type="button" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#exampleModal_5B" data-whatever="<?php echo $d0M . "*/*/*" . $d7M . "*/*/*" . $d8M . "*/*/*" . $d9M . "*/*/*" . $d10M; ?>">
					...
					<?php
						if ($d11M == 0) {
					?>
						<button type="button" class="btn btn-success btn-xs" data-toggle="modal" data-target="#exampleModal_5B" data-whatever="<?php echo utf8_encode($d0M . "*/*/*" . $d7M . "*/*/*" . $d8M . "*/*/*" . $d9M . "*/*/*" . $d10M); ?>">
							RESPUESTA PROCESADA
						</button>
	...
	<div class="modal fade" id="exampleModal_5B" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog" role="document" id="mdialTamanio_5">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">PQR(S)</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				...
				<div class="modal-body">
					<table class="table">
							...
							<td>
								<div class="form-row">
									<div class="form-group col-md-4">
										...
										<br>
										<textarea name="respqr" id="respqr" cols="90" rows="10" maxlength="1000000"></textarea>
									</div>
								</div>
							</td>
						</tr>
						...
						<tr>
							<td>
								<a id="radicar_pqr" title="DAR RESPUESTA">
	...
	<script>
		$(document).ready(function() {
			...
			$("#radicar_pqr").click(function(evento) {
				...
				if ($('#respqr').val() == null || $('#respqr').val().length == 0 || /^\s+$/.test($('#respqr').val())) {
					...
					if (nom_archivo == 0) {
						...
						if (confirm("ESTA SEGURO DE REALIZAR EL PROCESO")) {
							...
							$.ajax({
								...
								url: 'index.php?controller=archivo&action=Radicar_PQR',
								...
								setTimeout(function() {
									...
									//RECARGAR 
									location.href = 'index.php?controller=archivo&action=Consulta_PQR';
			...
			$('#exampleModal_5B').on('show.bs.modal', function(event) {
				...
				modal.find('.modal-title').text('ID PQR: ' + recipient_2[0] + ' - RESPUESTA PROCESADA: ' + msg_respuesta)
				...
				//CAMPOS OCULTOS
				$('#idmemoexterno').val(recipient_2[0]); //ID PQR


				//CARGA INFORMACION SI YA FUE PROCESADA LA RESPUESTA
				//Y DESEAN EDITARLA
				$('#respqr').val('');
				$('#respqr').val(recipient_2[1]);
			}) //FIN $('#exampleModal').on('show.bs.modal', function (event) {
			...
			$("#buscar_pro").click(function(evento) {
				...
				if (
					...
					location.href = "index.php?controller=archivo&action=Busquedad_PQR_Consulta&dato_0=" + dato_0 + "&dato_1=" + dato_1 + "&dato_2=" + dato_2 + "&datox2=" + datox2 + "&datox4=" + datox4;
				}
			});
