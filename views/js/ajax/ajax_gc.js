...
//********************************************************************************************
						//PARA EL MANEJO DE ACCIONES DE GESTION CALIFICACION
//********************************************************************************************
var z_raREVI           = 1;
var Filas_raREVI       = 0;
//var cadena_fechas = " ";

function Adicionar_REVI(){
	...
	//VALIDA SI UNA PREGUNTA YA FUE ADICIONADA
	var existenumtitulo_raREVI = Validar_Radicado_Tabla_REVI();

	//existenumtitulo_raREVI = 0;
	
	if(existenumtitulo_raREVI == 1){
		
		existenumtitulo_raREVI = 1;
		alert("PREGUNTA YA FUE ADICIONADA");
	}
	else{//1
		
	//RETORNA 1 SI NO ESTAN TODOS LOS DATOS COMPLETOS PARA ADICIONAR UN REGISTRO A LA TABLA
	var validarcampos_raREVI = Validar_Campos_Agregar_RAREVI();
	
	//validarcampos = 0;
	
	if(validarcampos_raREVI == 1){
		
		validarcampos_raREVI = 1;
	}
	else{//2
	
			//DATOS 
			
			var dato1_raREVI = document.getElementById('id_accion').value;
			var dato2_raREVI = document.getElementById('listaREVI').value;
			//var dato3_raREVI = document.getElementById('listaREVI').value+"-"+$("#listaREVI option:selected").text();
			var dato3_raREVI = $("#listaREVI option:selected").text();
			var dato4_raREVI = document.getElementById('listaREVI2').value;
			var dato5_raREVI = document.getElementById('gc_OB_REVI').value;
			...
			//Filas = resultado.length;
			Filas_raREVI = 1;
			var cantidad_filas_raREVI;
			var TABLA_raREVI      = document.getElementById('t_raREVI');
			cantidad_filas_raREVI = TABLA_raREVI.rows.length;
	
			//alert(cantidad_filas);
			
			if(cantidad_filas_raREVI > 1){
						
				//alert('cantidad > 1');
					
				//Eliminar_Tabla();
				
				var tabla_raREVI = document.getElementById('cont_raREVI').innerHTML;
					
				//for (var id=0; id<Filas; id++){
				
					//tabla=tabla.substring(0,(tabla.length-8)); 
					
					tabla_raREVI = reemplazarCadena("</table>", " ", tabla_raREVI);
					
					tabla_raREVI+='<tr>';
					
					
					tabla_raREVI+='<td>'+dato1_raREVI+'</td>';
					
					tabla_raREVI+='<td>'+dato2_raREVI+'</td>';
					
					tabla_raREVI+='<td>'+dato3_raREVI+'</td>';
					
					tabla_raREVI+='<td>'+dato4_raREVI+'</td>';
					
					tabla_raREVI+='<td>'+dato5_raREVI+'</td>';
				
					
					tabla_raREVI+='<td><button type=button name=eliminarreparto id=eliminarreparto onclick="Eliminar_Fila_RAREVI(this.parentNode.parentNode.rowIndex)"><img src="views/images/eliminar.png" width="20" height="20" title="Eliminar Fila"/></button></td>';
										
					tabla_raREVI+='</tr></table>';
					
					document.getElementById('cont_raREVI').innerHTML = tabla_raREVI;
					
					z_raREVI++;
					
					Limpiar_Campos_3_RAREVI();
				 //}
			}
...
function Validar_Radicado_Tabla_REVI(){
	
	var existe = 0;
	
	var datonumero = document.getElementById('listaREVI').value;
	
	$('#t_raREVI tr').each(function () {
	
		var d0  = $(this).find("td").eq(1).html();
...
function Validar_Campos_Agregar_RAREVI(){
	
	var validar = 0;
	

	valor  = document.getElementById('id_accion').value;
	valor2 = document.getElementById('listaREVI').value;
	valor3 = document.getElementById('listaREVI2').value;
	valor4 = document.getElementById('gc_OB_REVI').value;
	...
	if( valor2 == null || valor2.length == 0 || /^\s+$/.test(valor2) ) {
		...
		alert("Defina Pregunta");
		document.getElementById('listaREVI').style.borderColor = '#FF0000';
		validar = 1;
		return validar;
	}
	...
	if( valor3 == null || valor3.length == 0 || /^\s+$/.test(valor3) ) {
		...
		alert("Defina SI / NO");
		document.getElementById('listaREVI2').style.borderColor = '#FF0000';
		validar = 1;
		return validar;
	}
	...
	if( valor4 == null || valor4.length == 0 || /^\s+$/.test(valor4) ) {
		...
		document.getElementById('gc_OB_REVI').style.borderColor = '#FF0000';
		validar = 1;
		return validar;
	}
...
function Limpiar_Campos_3_RAREVI(){
	
	

	document.getElementById('listaREVI').selectedIndex = 0;
	document.getElementById('listaREVI').style.borderColor='#E0E0E0';
	
	document.getElementById('listaREVI2').selectedIndex = 0;
	document.getElementById('listaREVI2').style.borderColor='#E0E0E0';
	
	document.getElementById('gc_OB_REVI').value = "";
	document.getElementById('gc_OB_REVI').style.borderColor='#E0E0E0';	
	
	
}

function Eliminar_Fila_RAREVI(idfila){
...
//********************************************************************************************
						//FIN PARA EL MANEJO DE ACCIONES DE GESTION CALIFICACION
//********************************************************************************************
