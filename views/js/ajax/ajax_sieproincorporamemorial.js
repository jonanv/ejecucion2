$(function(){
	...
	$(".btn_validar").click(function() {
		...
		if(cantidad_filas_2 > 1){
			...
			alert("No es Posible Realizar el Registro, no se Cuenta con Informacion en la Tabla MEMORIALES A INCORPORAR AL PROCESO");
			return false;
		}
    ...
    $("#frm").validate({
        ...
        submitHandler: function(form){
			...
			var dataString = 'radicado='+$('#radicado').val()+'&datospartes='+$('#datospartes').val()+'&idradicado='+$('#idr').val();;
			var url1       = "index.php?controller=archivo&action=Registrar_Incorpora_Memorial";
			//**********************************************************************************************************
	...
	$('#consultarincorporar').click(function(evento){
		...
		if( 
		   ...
		   fechafe    == null || fechafe.length    == 0 || /^\s+$/.test(fechafe) 
			...
			alert("Defina Radicado y Fechas");
			...
			$.get("funciones/traer_datos_incorpora_proceso.php?idradicado="+idradicado+"&fechaie="+fechaie+"&fechafe="+fechafe, function(datosradicado){
						...
						$("#idr").val(datosid_b[0]);
						$("#cedula_demandante").val(datosid_b[1]);
						$("#demandante").val(datosid_b[2]);
						$("#cedula_demandado").val(datosid_b[3]);
						$("#demandado").val(datosid_b[4]);
						$("#claseproceso").val(datosid_b[5]);
						$("#jo").val(datosid_b[6]);
						$("#jd").val(datosid_b[7]);
						...
						/*var idr               = datosid[0];
						var cedula_demandante = datosid[1];
						var demandante        = datosid[2];
						var cedula_demandado  = datosid[3];
						var demandado         = datosid[4];
						var claseproceso      = datosid[5];
						var jo                = datosid[6];
						var jd                = datosid[7];
						...
						location.href="index.php?controller=archivo&action=FiltroTablaIncorporar_Memorial&dato_0="+dato_0+"&dato_1="+idradicado+"&dato_2="+fechaie+"&dato_3="+fechafe+"&dato_4="+idr+"&dato_5="+cedula_demandante+"&dato_6="+demandante+"&dato_7="+cedula_demandado+"&dato_8="+demandado+"&dato_9="+claseproceso+"&dato_10="+jo+"&dato_11="+jd;*/
...
function Adicionar_Parte(idaccion,idmemorial){
	...
	var dx13 = document.getElementById("t2").rows[idmemorial].cells[13].innerText;
	...
	existenumtitulo = Existe_Numero(dx1);
	
	if(existenumtitulo == 1){
		
		existenumtitulo = 1;
		alert("Ya EXiste ese Id de Memorial en la Tabla no es posible su Adicion");
	}
	else{//1
	...
	if(validarcampos == 1){
		...
		validarcampos = 1;
			...
			var dato4  = document.getElementById('idr').value;
			
			var dato13 = dx13;//incorporado
			
			var dato5  = document.getElementById('a_despacho').value;
...
function Validar_Campos_Agregar(idaccion){
	...
	valor_2  = document.getElementById('a_despacho').value;
	...
	if( 
	   ...
	   valor_2 == null || valor_2.length == 0 || /^\s+$/.test(valor_2)
		...
		alert("Defina Observacion y A Despacho");
		document.getElementById('obserie').style.borderColor = '#FF0000';
		document.getElementById('a_despacho').style.borderColor = '#FF0000';
		validar = 1;
		return validar;
	}
...
function Adicionar_Memorial(datostabla){
			...
			if(cantidad_filas>1){
				...
				for (var id=0; id < Filas-1; id++){
					...
					tabla+='<td>'+resultado2[14]+'</td>';
					
					tabla+='<td>'+resultado2[15]+'</td>';
					
					tabla+='<td>'+resultado2[16]+'</td>';
					
					tabla+='<td>'+resultado2[17]+'</td>';
					
					tabla+='<td>'+resultado2[18]+'</td>';
					
					tabla+='<td>'+resultado2[19]+'</td>';
					
					tabla+='<td>'+resultado2[20]+'</td>';
					
					tabla+='<td>'+resultado2[21]+'</td>';
					
					tabla+='<td>'+resultado2[22]+'</td>';
					
					tabla+='<td>'+resultado2[23]+'</td>';
					
					
					tabla+='<td><button type=button name=btdireccion id=btdireccion onclick="Adicionar_Parte(1,this.parentNode.parentNode.rowIndex)" style="border-style:none; background-color:#FFFFFF"><img src="views/images/flecha.jpg" width="40" height="40" title="ADICIONAR MEMORIAL"/></button></td>';
...
function Limpiar_Campos(){
	...
	document.getElementById('fechaie').value = "";
	document.getElementById('fechaie').style.borderColor='#E0E0E0';
	
	document.getElementById('fechafe').value = "";
	document.getElementById('fechafe').style.borderColor='#E0E0E0';
