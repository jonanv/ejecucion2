...
$(function(){
		...
		if(cantidad_filas_2 > 1){
			...
			$('#t tr').each(function () {
				...
				if(controlemcabezados == 0){
					...
					datospartes = datospartes+"******"+d0+"//////"+d1+"//////"+d2+"//////"+d3+"//////"+d4;
			...
			alert("No es Posible Realizar el Registro, no se Cuenta con Informacion en la Tabla de ADICIONAR TITULOS");
			return false;
		}
		
		
		//NUEVO ADICIONADO EL 24 DE FEBRERO 2021
		//VALIDAR ARCHIVO
    ...
    $("#frm").validate({
        ...
        submitHandler: function(form){
			...
			//ASI ESTABA ANTES AL 24 DE FEBRERO 2021
			//var radicado       = $('#radicado').val();
			
			//NUEVO ADICIONADO EL 24 DE FEBRERO 2021
			//ADICIONAR CAMPOS A DATA
			//var datos_encabezado = $('#radicado').val()+"//////"+$('#oficiox').val();
			//$('#datos_encabezado').val(datos_encabezado);
			//data.append('datos_encabezado',$('#datos_encabezado').val());
			
			data.append('radicado',$('#radicado').val());
			data.append('datospartes',$('#datospartes').val());
			data.append('ordenadooficio',$('#oficiox').val());
			data.append('sin_archivo',$('#sin_archivo').val());//IDENTIFICA SI EL USUARIO DEFINE UN ARCHIVO 0 = NO - 1 = SI
			...
			var url1       = "index.php?controller=archivo&action=Listar_Titulos_OtrosJuzgados";
			//**********************************************************************************************************
			...
			$.ajax({
				...
				$('.mensage_otros').html(msg);//A el div con la clase msg, le insertamos el mensaje en formato  thml
				$('.mensage_otros').show('slow');//Mostramos el div.
											
				//DESAPARECER
				setTimeout(function() {
												
					$(".mensage_otros").fadeOut(1500);
												
					//RECARGAR 
					location.href='index.php?controller=archivo&action=Listar_Titulos_OtrosJuzgados';
            ...
            /*$.ajax({
                ...
                success: function(data){
					...
					$('.mensage_otros').html(data);//A el div con la clase msg, le insertamos el mensaje en formato  thml
					$('.mensage_otros').show('slow');//Mostramos el div.
...
function Adicionar_Parte(idaccion){
	...
	if(validarcampos == 1){
		...
		validarcampos = 1;
			...
			var dato5 = document.getElementById('fechap').value;
...
function Validar_Campos_Agregar(idaccion){
	...
	//valor5 = document.getElementById('fechap').value;
	
	
	
	if( valor == null || valor.length == 0 || /^\s+$/.test(valor) ) {
  		
		alert("Defina Numero de Orden");
...
function Limpiar_Campos(){
	...
	document.getElementById('numerotitulo').value = "";
	...
	document.getElementById('fechap').style.borderColor='#E0E0E0';
