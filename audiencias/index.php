	...
	function formatDate (input) {
		...
		$(document).ready(function()
		{
			...
			$(document).on("click",'a.modal',function(e) 
			{
				...
				$('#mask').fadeIn(1000).html("<div id='nuevo_evento' class='window' rel='"+fecha+"'>PROGRAMACION DE AUDIENCIAS DEL "+formatDate(fecha)+"</h2><a href='#' class='close' rel='"+fecha+"'>&nbsp;</a><div id='respuesta'></div><div id='respuesta_form'></div></div>");
				$.ajax({
					type: "GET",
					url: "ajax_calendario.php",
					cache: false,
			...
			$(document).on("click",'.generarword',function (e){
			...
			$(document).on("click",'.cambiar_estado',function (e){
				...
				var fecha  = $(this).attr("fecha_dia");
				
				$.ajax({
					type: "GET",
					url: "ajax_calendario.php",
					cache: false,
					data: { id:id,estado:estado,fecha:fecha,accion:"cambiar_estado" }
				}).done(function( respuesta2 ) 
				{
					$("#respuesta").html(respuesta2);
					current_p.parent("p").fadeOut();
